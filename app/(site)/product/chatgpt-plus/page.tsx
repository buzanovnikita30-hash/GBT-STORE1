...
import { Check, Sparkles, Zap, ImageIcon, Puzzle } from "lucide-react";
import { chatgptProductFaq, products } from "@/lib/mockData";

const chatgpt = products.find((p) => p.id === "chatgpt")!;

const features = [
  { icon: Sparkles, text: "Доступ к GPT-4 и расширенным моделям" },
  { icon: Zap, text: "Быстрее ответы в часы пик" },
  { icon: ImageIcon, text: "Генерация изображений (DALL·E) по тарифу" },
  { icon: Puzzle, text: "Расширенные инструменты и сценарии использования" },
];

const flowSteps = [
  "Оформите заказ на ChatGPT Plus и оплатите картой РФ.",
  "Отправьте email аккаунта OpenAI — пароль не нужен.",
  "Мы пригласим ваш аккаунт в семью/подписку или активируем по партнёрской схеме.",
  "Проверьте статус Plus в настройках — обычно до 15 минут.",
];
  ...
  return (
    <div className="pb-20">
        ...
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <SectionReveal>
            ...
            <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
              Полный Plus на вашем аккаунте — без иностранной карты и смены логина.
              Вторая наша услуга — только Spotify Premium.
            </p>
            <div className="mt-10 flex flex-wrap justify-center gap-3">
              <Button size="lg" className="shadow-lg shadow-primary/25" asChild>
                <Link href="/order">Оформить Plus</Link>
              </Button>
              <Button size="lg" variant="outline" asChild>
                <Link href="/product/spotify">Spotify Premium</Link>
              </Button>
            </div>
          </SectionReveal>
        </div>
      ...
      <section className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <SectionReveal>
          <h2 className="font-heading text-2xl font-bold">Вопросы по Plus</h2>
        </SectionReveal>
        <SectionReveal className="mt-8" delay={0.1}>
          <Accordion type="single" collapsible className="w-full">
            {chatgptProductFaq.map((item, i) => (
              <AccordionItem key={item.q} value={`cgpt-${i}`}>
